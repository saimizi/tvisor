...
pub fn initialize(heap_pages: Option<usize>) -> Result<InitializedHeap, HeapInitError> {
    let heap_pages = heap_pages.unwrap_or(INITIAL_HEAP_PAGES);
    let arena_start = mm::allocate_contiguous_pages(heap_pages)?;
    let arena_end = arena_start
        .checked_add(heap_page_to_bytes(heap_pages) as u64)
    ...
    unsafe { GLOBAL_HEAP.initialize(arena_va, heap_page_to_bytes(heap_pages))? };
