...
- makes allocator-managed RAM accessible through the current EL2 identity
  map; and
- reclaims U-Boot and original-DTB pages only after their lifetimes end.

## 3. Allocator model

The first allocator uses 4 KiB pages and a bitmap stored in tvisor-owned
`.bss`. A bit records whether a usable physical page is currently allocated.
Usable-region membership remains authoritative: a clear bitmap bit outside a
usable region is not an allocatable page.

The Raspberry Pi 4 physical layout used by tvisor lies below 4 GiB, requiring
at most 1,048,576 page bits, or 128 KiB of bitmap storage. The implementation
must reject a usable page whose number is outside the bitmap's configured
physical aperture instead of truncating it.

Allocation is deterministic first-fit across normalized usable regions. Free
...
Handoff-only regions include:

- U-Boot relocated code, data, heap, stack, and translation tables;
- raw ELF staging bytes outside the live tvisor image;
- the original working DTB pages; and
...
3. all required platform and memory information has been copied into owned
   structures;
4. no executing code or dereferenced pointer depends on U-Boot or the original
   DTB; and
5. page-rounded reservation boundaries have been handled conservatively.

The current global FDT handle borrows the original DTB and cannot be revoked.
Until that API is changed or the DTB is copied into tvisor-owned storage, the
DTB pages remain reserved even though other U-Boot handoff pages may be
reclaimed.
...
The active page-table arena is excluded from allocation even though it was
selected from pre-takeover `INITIAL` RAM. Descriptor stores are published
...
- Extend `src/mm.rs` to map allocator-managed RAM and initialize runtime
  allocator state from owned region data.
...
- allocator bitmap and control state are inside mapped tvisor-owned data; and
- every allocator region has a Normal RW/XN EL2 mapping.

### Raspberry Pi 4

- initialize the allocator after the tvisor page-table switch;
